...
description: Internet and basic protocols and HW
type: tangibles
...
- Another example of a protocal from the presentation layer is MIME, it formats emails to send text and attachments, videos, or url's. Helps encode or decode data sent through emails
...
> Popcorn Hack
 ...
 ###### Answer
 - youtube: 142.250.72.174, github: 192.30.255.112, discord.com: 162.159.136.232, chatgpt: 172.64.155.141 (all found my nslookup in terminal)
 - Our subdomain last time for our passion project was pheonixapp
 - Our domain for this CPT project is spooketti.github.io
...
###### Answer
- Streaming is an example of UDP because it uses real-time data and speed delivery. 
- We might use TCP in web services, since there are data packets
...
1. Three of the four following protocols are on the same layer. Identify which ones and what layer they are on, and why they are on each layer:
    - ASCII (see above for information) | Presentation Layer, because it works with characters and encoding, like text to binary
    - FTP (facilitates transfer of files over the internet) | Presentation Layer, allows users to transport files over the internet
    - TLS (see HTTPS section) | Presentation Layer, because it secures info by encrypting data
    - USB (permits data exchange between electronics) | Physical Layer, it is a physical object that transfers data
...
It would belong on the Application layer of the OSI model because it provides a network or interface to users. The function of this layer is to share resources and get humans to interact
...
The domain bob should buy is best.com, which would be the main root or domain. The subdomains he should buy are is.the.best.com, the.best.com, bob.is.the.best.com 

#### HTTP and HTTPS
1. What is a difference between HTTP and HTTPS? 
<br>
 - What protocol does HTTPS use that HTTP doesn't?
 HTTP sends data over the internet, which could be vulnerable to being hacked, HTTPS encypts the data and provides security. HTTPS uses TLS and SSL for encrpytion.
...
<br>
 ...
 We used HTTP. The benefits is that HTTP is faster, its more simple and easy to use, and can be widely supported across platforms
...
<br>
 ...
 He should use UDP because there is a faster transmission with low latency. Some cons of this protocol are that it can be unreliable for delivering and can cause lag in the streaming. An example is when there is packet loss so it cause lag on the stream.
...
<br>
 ...
 It's important because it allows all packets to arrive properly, without any data being lost. An example is a TCP using sequence numbers (researched this) and makes sure all the data packets are received.
...
<br>
 ...
 Server B can make sure it receives any packets before Server A sends more in TCP by sending ACK messages, (acknolwedgement messages). For UDP, there is not really a way to ensure the transfer. Another use of this is emailing or streaming